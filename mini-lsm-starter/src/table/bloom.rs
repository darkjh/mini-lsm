...
impl Bloom {
    ...
    pub fn decode(buf: &[u8]) -> Result<Self> {
        ...
        if checksum != crc32fast::hash(&buf[..end_offset - 4]) {
            bail!("Bloom filter checksum mismatch");
        }

        let bf_end = end_offset - 4;
        let filter = &buf[..bf_end - 1];
        let k = buf[bf_end - 1];
        Ok(Self {
    ...
    pub fn encode(&self, buf: &mut Vec<u8>) {
        let bf_start = buf.len();
        buf.extend(&self.filter);
        buf.put_u8(self.k);
        let bf_end = buf.len();

        let checksum = crc32fast::hash(&buf[bf_start..bf_end]);
